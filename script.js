(function(){
  ...
  document.getElementById("btnProfit").onclick = loadProfitAnalysisView;
  ...
  function getCommissionRate() {
    return parseFloat(document.getElementById("commissionRate").value) || 0.0;
  }

  function getProfitTarget() {
    return parseFloat(document.getElementById("profitTarget").value) || 0.00;
  }
  ...
  function loadProfitAnalysisView() {
    activateTab("btnProfit");
    view.innerHTML = `
      <section class="section">
        <h2>📊 Profit Analysis: Purchases & Sales</h2>
        
        <div class="profit-analysis-cards">
          <!-- Purchases Card -->
          <div class="profit-card purchases-card">
            <h4>🟩 Purchases</h4>
            <div class="form-group">
              <label for="sharesPurchased">Number of Shares Purchased:</label>
              <input id="sharesPurchased" type="number" placeholder="e.g. 1000" min="1">
            </div>
            <div class="form-group">
              <label for="buyingPrice">Buying Price per Share (₦):</label>
              <input id="buyingPrice" type="number" placeholder="e.g. 5.50" step="0.01" min="0.01">
            </div>
            <div class="form-group">
              <label for="actualPurchaseAmount">Actual Purchase Amount (₦):</label>
              <input id="actualPurchaseAmount" type="number" placeholder="Auto-calculated" readonly>
            </div>
            <div class="form-group">
              <label for="totalCostPrice">Total purchase Price (₦):</label>
              <input id="totalCostPrice" type="number" placeholder="input all costs" step="0.01" min="0">
            </div>
            <div class="charges-display">
              <div class="charge-item">
                <span>Purchase Charges (₦): </span>
                <span id="purchaseChargesAmount">0.00</span>
              </div>
              <div class="charge-item">
                <span>Purchase Charges (%): </span>
                <span id="purchaseChargesPercent">0.00%</span>
              </div>
            </div>
          </div>
          
          <!-- Sales Card -->
          <div class="profit-card sales-card">
            <h4>🟦 Sales</h4>
            <div class="form-group">
              <label for="sharesSold">Number of Shares Sold:</label>
              <input id="sharesSold" type="number" placeholder="e.g. 1000" min="1">
            </div>
            <div class="form-group">
              <label for="sellingPrice">Selling Price per Share (₦):</label>
              <input id="sellingPrice" type="number" placeholder="e.g. 6.00" step="0.01" min="0.01">
            </div>
            <div class="form-group">
              <label for="actualSaleAmount">Actual Sale Amount (₦):</label>
              <input id="actualSaleAmount" type="number" placeholder="Auto-calculated" readonly>
            </div>
            <div class="form-group">
              <label for="totalSalePrice">Total Sale Price (₦):</label>
              <input id="totalSalePrice" type="number" placeholder="input all costs" step="0.01" min="0">
            </div>
            <div class="charges-display">
              <div class="charge-item">
                <span>Sales Charges (₦): </span>
                <span id="salesChargesAmount">0.00</span>
              </div>
              <div class="charge-item">
                <span>Sales Charges (%): </span>
                <span id="salesChargesPercent">0.00%</span>
              </div>
            </div>
          </div>
        </div>
        
        <button class="calc profit-calc-btn" onclick="calculateProfit()">Calculate Profit</button>
        <div id="profitAnalysisResult"></div>
      </section>`;
    setupProfitAnalysisEventListeners();
  }
  ...
  window.multiCalc = function() {
    ...
    for (let i = 0; i < 4; i++) {
      const name = document.getElementById(`name${i}`).value || `Result for Stock  ${i + 1}`;
    ...
    results.forEach((r, idx) => {
      ...
      div.innerHTML = `
        ...
        <strong>Total Investment:</strong> ₦${r.investment.toLocaleString()}<br>
        <strong>Number of Shares:</strong> ${r.shares.toLocaleString()}<br>
        <strong>Buying Price:</strong> ₦${r.buyPrice.toFixed(2)} <br> <strong>Selling Price:</strong> ₦${r.sellPrice.toFixed(2)}<br>
  ...
  // Profit Analysis Event Listeners and Functions
  function setupProfitAnalysisEventListeners() {
    setTimeout(() => {
      // Purchase inputs
      const sharesPurchased = document.getElementById('sharesPurchased');
      const buyingPrice = document.getElementById('buyingPrice');
      const totalCostPrice = document.getElementById('totalCostPrice');
      
      // Sales inputs
      const sharesSold = document.getElementById('sharesSold');
      const sellingPrice = document.getElementById('sellingPrice');
      const totalSalePrice = document.getElementById('totalSalePrice');
      
      if (sharesPurchased && buyingPrice) {
        sharesPurchased.addEventListener('input', updatePurchaseCalculations);
        buyingPrice.addEventListener('input', updatePurchaseCalculations);
        totalCostPrice.addEventListener('input', updatePurchaseCalculations);
      }
      
      if (sharesSold && sellingPrice) {
        sharesSold.addEventListener('input', updateSalesCalculations);
        sellingPrice.addEventListener('input', updateSalesCalculations);
        totalSalePrice.addEventListener('input', updateSalesCalculations);
      }
      
      // Add event listeners to save data
      const profitInputs = document.querySelectorAll('.profit-analysis-section input');
      profitInputs.forEach(input => {
        input.addEventListener('input', saveProfitAnalysisData);
      });
      
      // Restore saved data
      restoreProfitAnalysisData();
    }, 100);
  }

  function updatePurchaseCalculations() {
    const sharesPurchased = parseFloat(document.getElementById('sharesPurchased').value) || 0;
    const buyingPrice = parseFloat(document.getElementById('buyingPrice').value) || 0;
    const totalCostPrice = parseFloat(document.getElementById('totalCostPrice').value) || 0;
    
    // Calculate actual purchase amount
    const actualPurchaseAmount = sharesPurchased * buyingPrice;
    document.getElementById('actualPurchaseAmount').value = actualPurchaseAmount.toFixed(2);
    
    // Calculate purchase charges
    const purchaseCharges = totalCostPrice - actualPurchaseAmount;
    const purchaseChargesPercent = actualPurchaseAmount > 0 ? (purchaseCharges / actualPurchaseAmount) * 100 : 0;
    
    document.getElementById('purchaseChargesAmount').textContent = purchaseCharges.toFixed(2);
    document.getElementById('purchaseChargesPercent').textContent = purchaseChargesPercent.toFixed(2) + '%';
  }

  function updateSalesCalculations() {
    const sharesSold = parseFloat(document.getElementById('sharesSold').value) || 0;
    const sellingPrice = parseFloat(document.getElementById('sellingPrice').value) || 0;
    const totalSalePrice = parseFloat(document.getElementById('totalSalePrice').value) || 0;
    
    // Calculate actual sale amount
    const actualSaleAmount = sharesSold * sellingPrice;
    document.getElementById('actualSaleAmount').value = actualSaleAmount.toFixed(2);
    
    // Calculate sales charges - ensure they are positive
    const salesCharges = Math.abs(actualSaleAmount - totalSalePrice);
    const salesChargesPercent = actualSaleAmount > 0 ? (salesCharges / actualSaleAmount) * 100 : 0;
    
    document.getElementById('salesChargesAmount').textContent = salesCharges.toFixed(2);
    document.getElementById('salesChargesPercent').textContent = salesChargesPercent.toFixed(2) + '%';
  }
  ...
  window.calculateProfit = function() {
    ...
    // Validation
    if (!sharesPurchased || !buyingPrice || !totalCostPrice) {
      alert('Please fill in all purchase fields');
      return;
    }
    
    if (!sharesSold || !sellingPrice || !totalSalePrice) {
      alert('Please fill in all sales fields');
      return;
    }
    
    if (sharesSold > sharesPurchased) {
      alert('Cannot sell more shares than purchased');
      return;
    }
    ...
    const purchaseCharges = totalCostPrice - actualPurchaseAmount;
    const salesCharges = Math.abs(actualSaleAmount - totalSalePrice);
    const purchaseChargesPercent = (purchaseCharges / actualPurchaseAmount) * 100;
    const salesChargesPercent = (salesCharges / actualSaleAmount) * 100;
    
    const totalInvestment = totalCostPrice;
    const totalRevenue = totalSalePrice;
    const grossProfit = totalRevenue - actualPurchaseAmount;
    const netProfit = totalRevenue - totalCostPrice;
    const roi = (netProfit / totalInvestment) * 100;
    
    // Performance indicator
    const performanceIndicator = roi > 15 ? '🚀' : roi > 5 ? '📈' : roi > 0 ? '📊' : '📉';
    const performanceText = roi > 15 ? 'Excellent Performance' : roi > 5 ? 'Good Performance' : roi > 0 ? 'Moderate Performance' : 'Poor Performance';
    
    // Display results
    const resultHTML = `
      <div class="card profit-result-card ${roi > 10 ? 'best' : ''}">
        <div class="performance-indicator">${performanceIndicator}</div>
        <strong>Profit Analysis Results - ${performanceText}</strong><br><br>
        
        <div class="result-grid">
          <div class="result-section">
            <strong>📊 Trading Summary:</strong><br>
            Number of Shares Bought: ${sharesPurchased.toLocaleString()}<br>
            Number of Shares Sold: ${sharesSold.toLocaleString()}<br>
            Buy Price per Share: ₦${buyingPrice.toFixed(2)}<br>
            Sell Price per Share: ₦${sellingPrice.toFixed(2)}<br><br>
          </div>
          
          <div class="result-section">
            <strong>💰 Financial Summary:</strong><br>
            Total Investment: ₦${totalInvestment.toLocaleString()}<br>
            Total Revenue: ₦${totalRevenue.toLocaleString()}<br>
            Gross Profit: ₦${grossProfit.toLocaleString()}<br><br>
          </div>
          
          <div class="result-section">
            <strong>📋 Charges Breakdown:</strong><br>
            Purchase Charges: ₦${purchaseCharges.toFixed(2)} (${purchaseChargesPercent.toFixed(2)}%)<br>
            Sales Charges: ₦${salesCharges.toFixed(2)} (${salesChargesPercent.toFixed(2)}%)<br>
            Total Charges: ₦${(purchaseCharges + salesCharges).toFixed(2)}<br><br>
          </div>
          
          <div class="result-section final-results">
    ...
    document.getElementById('profitAnalysisResult').innerHTML = resultHTML;
    
    // Save results
    saveProfitAnalysisResults({
      sharesPurchased,
      buyingPrice,
      totalCostPrice,
      sharesSold,
  ...
  // localStorage functions for Profit Analysis
  function saveProfitAnalysisData() {
    const profitAnalysisData = {
      sharesPurchased: document.getElementById('sharesPurchased')?.value || '',
      buyingPrice: document.getElementById('buyingPrice')?.value || '',
      totalCostPrice: document.getElementById('totalCostPrice')?.value || '',
      sharesSold: document.getElementById('sharesSold')?.value || '',
      sellingPrice: document.getElementById('sellingPrice')?.value || '',
      totalSalePrice: document.getElementById('totalSalePrice')?.value || ''
    };
    localStorage.setItem('profitAnalysisData', JSON.stringify(profitAnalysisData));
  }

  function saveProfitAnalysisResults(results) {
    localStorage.setItem('profitAnalysisResults', JSON.stringify(results));
  }

  function restoreProfitAnalysisData() {
    const savedData = localStorage.getItem('profitAnalysisData');
    const savedResults = localStorage.getItem('profitAnalysisResults');
    ...
    if (savedData) {
      ...
      // Restore input values
      setTimeout(() => {
        if (data.sharesPurchased && document.getElementById('sharesPurchased')) {
          document.getElementById('sharesPurchased').value = data.sharesPurchased;
        }
        if (data.buyingPrice && document.getElementById('buyingPrice')) {
          document.getElementById('buyingPrice').value = data.buyingPrice;
        }
        if (data.totalCostPrice && document.getElementById('totalCostPrice')) {
          document.getElementById('totalCostPrice').value = data.totalCostPrice;
        }
        if (data.sharesSold && document.getElementById('sharesSold')) {
          document.getElementById('sharesSold').value = data.sharesSold;
        }
        if (data.sellingPrice && document.getElementById('sellingPrice')) {
          document.getElementById('sellingPrice').value = data.sellingPrice;
        }
        if (data.totalSalePrice && document.getElementById('totalSalePrice')) {
          document.getElementById('totalSalePrice').value = data.totalSalePrice;
        }
    ...
    if (savedResults) {
      const results = JSON.parse(savedResults);
      if (results.resultHTML && document.getElementById('profitAnalysisResult')) {
        document.getElementById('profitAnalysisResult').innerHTML = results.resultHTML;
        
        // Recreate chart if results exist
        setTimeout(() => {
